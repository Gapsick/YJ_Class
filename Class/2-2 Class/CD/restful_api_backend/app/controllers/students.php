...
// GET /students/{std_id}
function students_show(string $std_id): void {
    try {
        if (!ctype_digit($std_id)) { json_response(['error'=>'잘못된 ID'], 400); return; }
        $db = get_db();
...
// POST /students   {std_id, email, password, name, birth, gender, admission_year, current_year, status}
// POST /students   {std_id, email, password, name, birth(YYYY-MM-DD), gender('M'|'F'),
//                   admission_year(YYYY), current_year(1~4), status('재학'|'휴학'|'졸업'|'제적'|'자퇴' 선택, 기본: '재학')}
...
// PUT /students/{std_id}
function students_update(string $std_id): void {
    $db   = get_db();
    $in   = read_json_body();
    $sets = [];
...
// DELETE /students/{std_id}
function students_delete(string $std_id): void {
    $db = get_db();
    $e_std_id = $db->real_escape_string($std_id);
